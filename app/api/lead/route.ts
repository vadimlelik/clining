...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Некорректный формат данных. Ожидается JSON." }, { status: 400 });
    }
    ...
    if (sendTasks.length === 0) {
      return NextResponse.json(
        {
          error:
            "Сервер не настроен для отправки заявок. Добавьте TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID или CRM_WEBHOOK_URL в переменные окружения.",
        },
        { status: 503 },
      );
    }
